...
class Persistor(object):
    ...
    def persist(self, model_directory, model_name,tenanetId,botId):
        # type: (Text) -> None
        ...
        file_key, tar_path = self._compress(
                model_directory, model_name,tenanetId,botId)
        self._persist_tar(file_key, tar_path)

    def retrieve(self, model_name, tenanetId,botId, target_path):
        # type: (Text) -> None
        """Downloads a model that has been persisted to cloud storage."""

        tar_name = self._tar_name(model_name, tenanetId,botId)

        self._retrieve_tar(tar_name)
        self._decompress(tar_name, target_path)

    def list_models(self, tenanetId ,botId):
        # type: (Text) -> List[Text]
    ...
    def _compress(self, model_directory, model_name,tenanetId,botId):
        # type: (Text) -> Tuple[Text, Text]
        """Creates a compressed archive and returns key and tar."""
        import tempfile

        dirpath = tempfile.mkdtemp()
        base_name = self._tar_name(model_name, tenanetId,botId, include_extension=False)
    ...
    @staticmethod
    def _project_prefix(tenanetId ,botId):
        # type: (Text) -> Text
        return '{tenanet}_{bot}'.format(tenanet=tenanetId,bot=botId)
    ...
    @staticmethod
    def _tar_name(model_name, tenanetId,botId, include_extension=True):
        # type: (Text, Text, bool) -> Text

        ext = ".tar.gz" if include_extension else ""
        return '{p}{m}{ext}'.format(p=Persistor._project_prefix(tenanetId,botId),
...
class AWSPersistor(Persistor):
    ...
    def list_models(self, tenanetId ,botId):
        # type: (Text) -> List[Text]
        try:
            prefix = self._project_prefix(tenanetId ,botId)
            return [self._project_and_model_from_filename(obj.key)[1]
                    for obj in self.bucket.objects.filter(Prefix=prefix)]
        except Exception as e:
            logger.warn("Failed to list models for tenanetId {} in "
                        "AWS. {}".format(tenanetId, e))
            return []
...
class GCSPersistor(Persistor):
    ...
    def list_models(self, tenanetId ,botId):
        ...
        try:
            ...
            logger.warn("Failed to list models for tenanetId {} in "
                        "google cloud storage. {}".format(tenanetId, e))
            return []
...
class AzurePersistor(Persistor):
    ...
    def list_models(self, tenanetId ,botId):
        ...
        try:
            ...
            logger.warning("Failed to list models for tenanetId {} in "
                           "azure blob storage. {}".format(tenanetId, e))
            return []
